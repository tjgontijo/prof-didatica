    ...
    <div className="mb-6">
      ...
      <div className="space-y-3">
        {sortedOrderBumps.length === 0 ? (
          <p className="text-sm text-gray-500 text-center py-4">Nenhum item adicional disponível no momento.</p>
        ) : (
          ...
          const desconto = item.percentDesconto || 
              ...
              <div className="px-4 py-3 border-t border-[#EEE] bg-emerald-200 rounded-b-[6px]">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-[14px] font-medium text-[#333] flex items-center gap-2">
                    {loadingId === item.id ? (
                      <>
                        <svg className="animate-spin h-4 w-4 text-[#00A859]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      ...
                      item.selecionado ? 'Remover do pedido' : 'Adicionar ao pedido'
                    )}
                  </span>
                </label>
              </div>
