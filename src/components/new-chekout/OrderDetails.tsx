    ...
    <div key={bump.id} className="flex justify-between items-center py-1">
      <div className="flex flex-col">
        <span className="text-[14px] text-[#333]">
          {bump.name.length > 35 ? `${bump.name.substring(0, 35).trim()}...` : bump.name}
        </span>
    ...
    <div className="mb-6">
      ...
      <div className="space-y-3 bg-[#F8F9FA] rounded-[6px] p-4">
        ...
        <div className="flex justify-between items-center py-1">
          <span className="text-[14px] text-[#333] font-bold">{produto.name}</span>
        ...
        {orderBumpsSelecionados.map((bump) => (
          <OrderBumpItem key={bump.id} bump={bump} />
        ))}
